import { RestoreWallets, setWalletAndFetchData } from "../utils/lib";
...
export default function Restore() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("in handleSubmit of restore");
    if (passwordRef.current.value !== confirmRef.current.value) {
      console.log("in error of restore");

      dispatch({ type: "SET_ERROR", param: "passwords do not match" });
      return;
    } else {
      try {
        const wallet = await RestoreWallets(
          nameRef.current.value,
          mnemonicRef.current.value,
        );
        console.log("restore:", wallet);
        // encrypt wallet (SHA256) and save to localStorage
        storage.encryptObjectToLocalStorage(
          wallet,
          passwordRef.current.value,
          nameRef.current.value,
        );
        setWalletAndFetchData(wallet, dispatch);
