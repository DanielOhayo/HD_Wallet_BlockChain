...
export const saveWallet = async (wallet, password, key) => {
    ...
    const tempWallet = {
        name: wallet.name,
        mainnet: Object.entries(wallet.mainnet).map(([k, a]) => { return { coin: k, address: a.address } }),
        testnet: Object.entries(wallet.testnet).map(([k, a]) => { return { coin: k, address: a.address } })
    }
...
export const setTempWallet = (wallet) => {
    console.log('fullWallet', wallet)
    const w = {
        name: wallet.name,
        mainnet: Object.entries(wallet.mainnet).reduce((s, [k, a]) => {
            return { ...s, [k]: { coinInfo: a.coinInfo, address: a.address } }
        }, {}),
        testnet: Object.entries(wallet.testnet).reduce((s, [k, a]) => {
            return { ...s, [k]: { coinInfo: a.coinInfo, address: a.address } }
        }, {})
    }
